...
docker-compose -f docker-compose-app-green.yml up --build -d
# Set Django green app
docker exec web_server /bin/sh /etc/nginx/conf.d/nginx_update_green.sh
# Django green app if there is then down (could be not exist)
docker-compose -f docker-compose-app-blue.yml down

# After all Django blue and green is once `--build` then you can just use `up` without `--build`
# Deploy Django blue app
docker-compose -f docker-compose-app-blue.yml up -d
# Set Django green app
docker exec web_server /bin/sh /etc/nginx/conf.d/nginx_update_green.sh
```
